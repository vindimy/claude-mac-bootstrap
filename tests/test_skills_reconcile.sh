...
assert_contains "$(npx_log)" "skills add acme/tools -g -y -a claude-code codex -s alpha beta" "add command shape"
...
assert_contains "$(npx_log)" "skills add acme/tools -g -y -a claude-code codex -s *" "star passed literally"
...
assert_eq "claude-code codex" "$(skills_installed_agents)" "both agent homes present"
: >"$FAKE_NPX_LOG"
rm -rf "$HOME/.codex"
assert_eq "claude-code" "$(skills_installed_agents)" "codex home missing -> only claude-code"
skills_reconcile acme/suite "*" "one"
assert_contains "$(npx_log)" "skills add acme/suite -g -y -a claude-code -s one" "star agents -> installed agents only"
...
assert_contains "$out" "[dry-run] npx -y skills add acme/suite -g -y -a claude-code codex -s two" "dry-run echoes add"
