...
if command -v shellcheck >/dev/null 2>&1; then
  ...
  shellcheck lib/*.sh apps/*.sh run.sh update.sh install.sh bin/dropbox-ignore-git.sh bin/github-mcp.sh tests/lib.sh tests/run.sh tests/fakes/npx tests/fakes/code tests/fakes/fake-mcp-cli tests/fakes/claude tests/fakes/codex tests/fakes/gemini $tests || rc=1
else
